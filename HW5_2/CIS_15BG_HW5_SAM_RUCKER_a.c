...
//  Statements
    ...
    fprintf(stdout, "Please select file to open.\nsales or sales_2: ");
    scanf("%s", nameFile);
    FLUSH;
...
//  Statements
    ...
    for(i = 0; i < pHead->worker; i++)
    {
        ...
        sscanf(temp,"%s %[^;]%*s %[^\n]%*s", pHead->person[i].id, tempName, tempSales);

        count = strlen(tempName);

        pHead->person[i].name = nameAlloc(pHead, count);

        strcpy(pHead->person[i].name, tempName);
        ...
        {
            sscanf(ptr, "%d", &tempsale);
            pHead->person[i].sales[j] = tempsale;
            ptr += 3;
        }// inner for
